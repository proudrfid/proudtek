...
- [ ] **Step 3: Implement in `seo-content.ts`** — export (the "8–12 SKU" figure is retired; do not add it anywhere):
```ts
export function formatSamplePolicy(): string {
  return "Standard samples are free for qualified B2B buyers — typically 2–3 SKUs, 5–20 pieces across LF / HF / UHF; larger evaluation kits are sized for qualified pilots. Prepared in 1 business day; you cover DHL/FedEx freight ($25–60), refunded against your first production order. Custom samples (encoding, printing or a new housing) add 3–5 business days and a sample fee credited against the production PO.";
}
...
```
`HOME_EVIDENCE` in `home-v2.ts` stays as its three existing cards (verified certificate / company-stated production model / documentation pack).
...
- [ ] **Step 8: index.json** (editorial `/`) — "Direct factory pricing: no middleman markup…" → "Manufacturing partner: specification, chip sourcing and QC in-house; production on contracted partner lines in Shenzhen — BOM breakdown on request."; "Sample-first sales motion: … 5–7 working days" → "prepared in 1 business day"; FAQ production lead time "5–7 working days" → "7–15 business days from order confirmation for stock chip types" (canonical `leadTime.stockChip`); "Documentation discipline: RoHS, REACH, EU DPP, FCC, MIC and CE declarations issued per shipment" → "Documentation pack per shipment: sample-based RoHS, REACH SVHC and CE test reports (numbers on the certifications page), Certificate of Origin and the lot's functional test report; testing on your specification on request."; FAQ "How fast can samples arrive?" → aligned; FAQ "Do you provide compliance documentation?" → aligned (no "EU DPP technical files" as default).
...
- [ ] Regenerate editorial snapshot fixtures: `npx vitest run -u src/lib/__tests__/editorial-pages-integration.snapshot.test.ts src/lib/__tests__/editorial-pages-leaf.snapshot.test.ts src/lib/__tests__/editorial-pages-variants.snapshot.test.ts`, review `git diff --stat` on `.snap` files, commit.
