 ...
 *
 * Also EXPORTS `listTargets({ groups, slug })` so the parallel dispatcher
 * (scripts/image-audit-next.mjs) can reuse the exact same resolution without
 * shelling out / duplicating the walk logic.
 *
...
export const REPO_ROOT = path.resolve(SCRIPT_DIR, "..", "..", "..", ".."); // repo root
const ROOT = REPO_ROOT;
const EDITORIAL = path.join(ROOT, "src", "content", "editorial");
const PUBLIC = path.join(ROOT, "public");

export const DEFAULT_GROUPS = ["blog", "solutions", "guides", "products"];
...
/**
 * Resolve every page → hero/section image. Pure function (no I/O beyond reads),
 * returns the same shape the CLI prints with --json.
 * @param {{groups?: string[], slug?: string|null}} opts
 */
export function listTargets({ groups = DEFAULT_GROUPS, slug = null } = {}) {
  const targets = [];
  for (const group of groups) {
    const groupDir = path.join(EDITORIAL, group);
    for (const absFile of walkJson(groupDir)) {
      const file = path.relative(ROOT, absFile);
      const pageSlug = path.basename(absFile).replace(/\.json$/, "");
      if (slug && pageSlug !== slug) continue;
      ...
      try {
        ...
        targets.push({ slug: pageSlug, group, category, file, parseError: String(err) });
        continue;
      }
...
function main() {
  ...
  const targets = listTargets({ groups, slug: slugArg });
